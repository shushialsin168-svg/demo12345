...
function buildMessage(o: OrderPayload): string {
  ...
  if (o.deliveryFee && o.deliveryFee > 0) {
    linesArr.push(`Subtotal: ${formatPrice(o.total - o.deliveryFee)}`);
    linesArr.push(`Delivery fee: ${formatPrice(o.deliveryFee)}`);
  }
...
/** Send a plain text message. Defaults to the shop channel. */
export async function sendTelegramMessage(
  text: string,
  chatId: string | number = TELEGRAM_CHANNEL
): Promise<boolean> {
  try {
    ...
    const data = await res.json();
    if (!data.ok) console.warn("sendMessage failed:", data.description);
    return !!data.ok;
  } catch (e) {
...
/** Send a photo (data URL) with caption. Defaults to the shop channel. */
export async function sendTelegramPhoto(
  ...
  try {
    ...
    const data = await res.json();
    if (!data.ok) console.warn("sendPhoto failed:", data.description);
    return !!data.ok;
  } catch (e) {
...
/** Build a friendly receipt caption for the customer. */
function buildCustomerMessage(o: OrderPayload): string {
  const lines: string[] = [];
  lines.push("🧾 *វិក័យប័ត្រ — NT26 Coffee*");
  lines.push("");
  lines.push(`🙏 អរគុណ ${o.customerName}!`);
  lines.push(`លេខវិក័យប័ត្រ: \`${o.invoice}\``);
  lines.push(`🕒 ${o.createdAt.toLocaleString()}`);
  lines.push(
    `🚚 ${o.method === "pickup" ? "យកនៅហាងផ្ទាល់" : "ដឹកជញ្ជូនដល់ទីតាំង"}`
  );
  lines.push("");
  o.items.forEach((it, i) => {
    const unit = it.product.price + it.optionsPrice;
  ...
  lines.push("");
  if (o.deliveryFee && o.deliveryFee > 0) {
    lines.push(`ថ្លៃដឹកជញ្ជូន: ${formatPrice(o.deliveryFee)}`);
  }
  lines.push(`*💰 សរុប: ${formatPrice(o.total)}*`);
  lines.push("");
  lines.push("📞 +855 93 342 226");
  return lines.join("\n");
}
...
export async function submitOrder(order: OrderPayload, receiptImage?: string): Promise<boolean> {
  ...
  // 2) Also send the receipt directly to the customer's own Telegram chat
  //    (works when they opened the app inside Telegram AND have started the
  //    bot at least once — Telegram forbids bots from messaging first).
  if (order.tgUser?.id) {
    const customerMsg = buildCustomerMessage(order);
    let sentToUser = false;
    if (receiptImage) {
      sentToUser = await sendTelegramPhoto(receiptImage, customerMsg, order.tgUser.id);
    }
    if (!sentToUser) {
      await sendTelegramMessage(customerMsg, order.tgUser.id);
    }
  }
