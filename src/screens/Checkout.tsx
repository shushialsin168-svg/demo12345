...
// Delivery fee: 1,500៛ per km (rounded up per started km, minimum 1 km).
const DELIVERY_FEE_PER_KM = 1500;
export function calcDeliveryFee(distanceM: number): number {
  const km = Math.max(1, Math.ceil(distanceM / 1000));
  return km * DELIVERY_FEE_PER_KM;
}
...
export default function Checkout({
  ...
  // Delivery fee: 1500៛ per km when delivering.
  const deliveryFee =
    method === "delivery" && loc.kind === "ok" ? calcDeliveryFee(loc.distance) : 0;
        ...
        <div>
          ...
          <div className="mt-2 space-y-1 text-sm">
                ...
                <span className="text-gray-600">
                  ...
                  {loc.kind === "ok" && (
                    <span className="text-[11px] text-gray-400 ml-1">
                      ({Math.max(1, Math.ceil(loc.distance / 1000))}km × ៛1,500)
                    </span>
                  )}
                </span>
                <span className="font-medium">
                  {loc.kind === "ok" ? formatPrice(deliveryFee) : "..."}
                </span>
              </div>
            )}
            <div className="flex justify-between">
            ...
            <div className="flex justify-between font-bold">
              ...
              <span className="text-[#148c78]">{formatPrice(grandTotal)}</span>
            </div>
          </div>
      ...
      <div className="absolute left-0 right-0 bottom-0 p-3 bg-white/80 backdrop-blur border-t border-gray-100">
        <div className="bg-black rounded-full flex items-center justify-between p-1 pl-4">
          <div className="text-white">
            ...
            <div className="text-sm font-bold">{formatPrice(grandTotal)}</div>
          </div>
